...
ESX.RegisterServerCallback('lp_documents:submitDocument', function(source, cb, data)
...
ESX.RegisterServerCallback('lp_documents:getPlayerDocuments', function(source, cb)
    local xPlayer = ESX.GetPlayerFromId(source)
    local forms = {}
    if xPlayer ~= nil then
...
ESX.RegisterServerCallback('lp_documents:getPlayerDetails', function(source, cb)
    local xPlayer = ESX.GetPlayerFromId(source)
    local cb_data = nil
...
RegisterServerEvent('lp_documents:ShowToPlayer')
AddEventHandler('lp_documents:ShowToPlayer', function(targetID, aForm)

    TriggerClientEvent('lp_documents:viewDocument', targetID, aForm)

end)

RegisterServerEvent('lp_documents:CopyToPlayer')
AddEventHandler('lp_documents:CopyToPlayer', function(targetID, aForm)
            ...
            if id ~= nil then
                MySQL.Async.fetchAll('SELECT * FROM user_documents where id = @id', {['@id']=id}, function (result)
                    ...
                    if(result[1] ~= nil) then
                        db_form = result[1]
                        db_form.data = json.decode(result[1].data)
                        TriggerClientEvent('lp_documents:copyForm', _targetid, db_form)
