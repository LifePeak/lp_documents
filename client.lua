...
function CopyFormToPlayer(aPlayer)
    --TriggerServerEvent('lp_documents:CopyToPlayer', GetPlayerServerId(player), aDocument)
    TriggerServerEvent('lp_documents:CopyToPlayer', aPlayer, CURRENT_DOCUMENT)
...
function DeleteDocument(aDocument)
    ...
    ESX.TriggerServerCallback('lp_documents:deleteDocument', function (cb)
        if cb == true then
            --remove form_close
...
function CreateNewForm(aDocument)
    ...
    PlayerData = ESX.GetPlayerData()
    ESX.TriggerServerCallback('lp_documents:getPlayerDetails', function (cb_player_details)
...
function ShowDocument(aPlayer)
     --   print("ssss: " .. dump(aPlayer))
        TriggerServerEvent('lp_documents:ShowToPlayer', aPlayer, CURRENT_DOCUMENT)
        CURRENT_DOCUMENT = nil
        CloseMenu()
end

RegisterNetEvent('lp_documents:viewDocument')
AddEventHandler('lp_documents:viewDocument', function( data )
...
RegisterNetEvent('lp_documents:copyForm')
AddEventHandler('lp_documents:copyForm', function( data )
...
AddEventHandler('lp_documents:refreshDocuments', function()
    ESX.TriggerServerCallback('lp_documents:getPlayerDocuments', function (cb_forms)
        ...
        if cb_forms ~= nil then
            ...
            --print("Documents are now refreshed")
        else
            print ("Received nil from newely created scale object.")
        end
    end)
...
RegisterNUICallback('form_submit', function(data, cb)
   -- print("received: " .. dump(data))
    CloseMenu()
    ESX.TriggerServerCallback('lp_documents:submitDocument', function (cb_form)
        if cb_form ~= nil then
